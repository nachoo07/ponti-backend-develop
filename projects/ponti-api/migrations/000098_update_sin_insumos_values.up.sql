-- Migración para actualizar vista v3_workorder_list
-- Mantiene EXACTAMENTE la lógica de la 000097 + completa 0s para órdenes sin insumos
...
labor_and_supplies AS (
  -- Filas de LABORES (solo para workorders CON insumos) - MANTIENE 000097
  SELECT
    w.id,
    w.number,
    w.project_id,
  ...
  -- Solo workorders que TENGAN insumos asociados - MANTIENE 000097
  WHERE w.deleted_at IS NULL
    AND EXISTS (
  ...
  -- Filas de INSUMOS (modificado para completar 0s con labor cuando no hay insumos)
  SELECT
    w.id,
    w.number,
    w.project_id,
    ...
    lb.name AS labor_name,
    cat_lb.name AS labor_category_name,
    COALESCE(t.name, 'Labor') AS type_name,  -- Si no hay tipo, usar 'Labor'
    w.contractor,
    ws.surface_ha,
    COALESCE(s.name, '-') AS supply_name,  -- Si no hay insumo, usar '-'
    COALESCE(wi.total_used, ws.surface_ha) AS consumption,  -- Si no hay consumo, usar superficie
    COALESCE(cat.name, cat_lb.name) AS category_name,  -- Si no hay categoría, usar categoría labor
    CASE 
      WHEN wi.total_used IS NOT NULL THEN (wi.total_used / ws.surface_ha)::numeric(18,6)
      ELSE (ws.surface_ha / ws.surface_ha)::numeric(18,6)  -- Si no hay insumo, dosis = 1
    END AS dose_per_ha,
    COALESCE(s.price, lb.price) AS unit_price,  -- Si no hay precio insumo, usar precio labor
    CASE 
      WHEN wi.total_used IS NOT NULL THEN (s.price * (wi.total_used / ws.surface_ha))::numeric
      ELSE lb.price::numeric  -- Si no hay insumo, costo = precio labor
    END AS supply_cost_per_ha,
    CASE 
      WHEN wi.total_used IS NOT NULL THEN (wi.total_used * s.price)::numeric
      ELSE (ws.surface_ha * lb.price)::numeric  -- Si no hay insumo, total = superficie × precio labor
