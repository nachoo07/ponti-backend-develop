-- ========================================
-- MIGRACIÓN 000202: FIX Lot Metrics Field Total Surface (UP)
-- ========================================
--
-- Propósito: Permitir que la card de "Superficie total" se actualice al filtrar por campo.
-- Problema: v3_lot_metrics solo expone superficie total del proyecto.
-- Solución: agregar field_total_hectares y usarlo en el backend cuando se filtra por field_id.
-- Fecha: 2025-11-17
-- Autor: Sistema
--
BEGIN;
...
WITH base AS (
  ...
  GROUP BY f.project_id, f.id, l.id, l.name, l.hectares, l.tons, l.sowing_date
),
workorder_costs AS (
  SELECT
    lot_id,
...
lot_per_ha_values AS (
  ...
  LEFT JOIN field_totals ft ON ft.field_id = b.field_id
)
SELECT
  project_id,
  field_id,
  ...
  field_total_hectares
FROM lot_per_ha_values;

COMMENT ON VIEW public.v3_lot_metrics IS
'Métricas por lote. FIX 000202: expone field_total_hectares para poder filtrar por campo.';

-- v3_lot_list se recrea igual que en 000199 (se mantiene sin cambios estructurales)
...
'Lista de lotes. FIX 000202: se recrea tras actualizar v3_lot_metrics.';
