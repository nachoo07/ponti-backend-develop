-- ========================================
-- MIGRACIÓN 000172: FIX Administración y Arriendo usan % acordado (UP)
-- ========================================
--
-- Propósito: Corregir el cálculo de share_pct en las categorías "Administración y Estructura" 
--           y "Arriendo Capitalizable" para que usen el % acordado (share_pct_agreed) 
--           en lugar del % real basado en aportes.
-- Problema: PROBLEMA 3 - "Administración y Estructura: no toma los porcentajes de aportes 
--           por socio cargados en Clientes y Sociedades"
-- Solución: Modificar v3_investor_contribution_data_view para usar share_pct_agreed directamente
-- Fecha: 2024-07-29
-- Autor: Sistema
--
BEGIN;

-- Recrear v3_investor_contribution_data_view con FIX en administration_structure y capitalizable_lease
...
WITH project_base AS (
  SELECT
    ...
    COALESCE(f.lease_type_value, 0)::numeric AS lease_type_value,
    COALESCE(f.lease_type_id, 0) IN (1, 3, 4) AS lease_is_fixed,
    COALESCE(f.lease_type_value * SUM(lb.seeded_area_ha), 0)::numeric AS lease_fixed_total_usd
  ...
  GROUP BY p.id, p.name, c.id, c.name, camp.id, camp.name, p.admin_cost, f.lease_type_value, f.lease_type_id
),
investor_base AS (
  SELECT
    pb.project_id,
...
investor_real_contributions AS (
  SELECT
    ...
    (pb.lease_fixed_total_usd * ib.share_pct_agreed / 100)::numeric AS rent_real_usd,
    (pb.administration_total_usd * ib.share_pct_agreed / 100)::numeric AS administration_real_usd,
    (
      ...
      (pb.lease_fixed_total_usd * ib.share_pct_agreed / 100) +
      (pb.administration_total_usd * ib.share_pct_agreed / 100)
    )::numeric AS total_real_contribution_usd,
    ct.total_contributions_usd AS project_total_contributions_usd
  FROM investor_base ib
  JOIN project_base pb ON pb.project_id = ib.project_id
...
SELECT
  ...
  (
    ...
    FROM (
      ...
      -- ✅ FIX: Administración y Estructura - Usar share_pct_agreed
      SELECT
        ...
        (
          SELECT jsonb_agg(jsonb_build_object(
            ...
            'share_pct', ib.share_pct_agreed  -- ✅ FIX: Usar % acordado, no % real
      ...
      -- ✅ FIX: Arriendo Capitalizable - Usar share_pct_agreed
      SELECT
...
FROM project_base pb
JOIN category_totals cc ON cc.project_id = pb.project_id
ORDER BY pb.project_id;

COMMENT ON VIEW public.v3_investor_contribution_data_view IS 'Vista FINAL para informe de Aportes por Inversor. FIX 000172: Administración y Arriendo usan share_pct_agreed.';
