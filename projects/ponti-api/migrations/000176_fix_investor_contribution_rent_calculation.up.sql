-- ========================================
-- MIGRACIÓN 000176: FIX Investor Contribution Rent Calculation (UP)
-- ========================================
--
-- Propósito: Corregir cálculo de arriendo en v3_report_investor_contribution_categories
-- Problema: rent_capitalizable_total_usd usa lease_type_value × seeded_area_ha con LIMIT 1
--           Esto IGNORA:
--           - Tipo 1: % sobre INGRESO NETO (lease_type_percent)
--           - Tipo 4: FIJO + % sobre INGRESO NETO (lease_type_value + lease_type_percent)
--           - Múltiples campos con diferentes valores
-- Solución: Usar v3_lot_ssot.rent_per_ha_for_lot() que considera TODOS los componentes
-- Fecha: 2025-11-03
-- Autor: Sistema
--
-- Impacto: Control 9 pasará de ERROR a OK
--          Informe de Aportes mostrará arriendo correcto ($162.692 en lugar de $122.467 para Jujuy)
--
...
SELECT
  ...
  -- AGROQUÍMICOS (versión 000170: usa project_id, NO lot_id)
  (COALESCE((
  ...
  -- FERTILIZANTES (versión 000170: usa project_id, NO lot_id)
  (COALESCE((
    SELECT SUM(sm.quantity * s.price)
    FROM public.supply_movements sm
    JOIN public.supplies s ON s.id = sm.supply_id AND s.deleted_at IS NULL
  ...
  -- SEMILLA (versión 000170: usa project_id, NO lot_id)
  (COALESCE((
    SELECT SUM(sm.quantity * s.price)
    FROM public.supply_movements sm
    JOIN public.supplies s ON s.id = sm.supply_id AND s.deleted_at IS NULL
  ...
  -- LABORES GENERALES (versión 000170: usa lot_base CTE)
  COALESCE((
  ...
  -- SIEMBRA (versión 000170: usa lot_base CTE)
  COALESCE((
    SELECT SUM(lab.price * w.effective_area)
    FROM public.workorders w
    JOIN public.labors lab ON w.labor_id = lab.id AND lab.deleted_at IS NULL
  ...
  -- RIEGO (versión 000170: usa lot_base CTE)
  COALESCE((
    SELECT SUM(lab.price * w.effective_area)
    FROM public.workorders w
    JOIN public.labors lab ON w.labor_id = lab.id AND lab.deleted_at IS NULL
  ...
  -- ========================================
  -- FIX: ARRIENDO CAPITALIZABLE
  -- ========================================
  -- ANTES (INCORRECTO):
  --   f.lease_type_value * SUM(seeded_area_ha) ... LIMIT 1
  -- AHORA (CORRECTO):
  --   Σ(rent_per_ha_for_lot × hectares)
  --   Esto considera TODOS los tipos de arriendo:
  --   - Tipo 1: % INGRESO NETO
  --   - Tipo 2: % UTILIDAD
  --   - Tipo 3: ARRIENDO FIJO
  --   - Tipo 4: FIJO + % INGRESO NETO
  -- ========================================
  COALESCE((
...
'Categorías de aporte por proyecto para informe de inversores. FIX 000176: Usa v3_lot_ssot.rent_per_ha_for_lot() para arriendo correcto.';
...
COMMENT ON VIEW public.v3_investor_contribution_data_view IS 'Vista FINAL para informe de Aportes por Inversor. FIX 000176: Arriendo usa rent_per_ha_for_lot().';
