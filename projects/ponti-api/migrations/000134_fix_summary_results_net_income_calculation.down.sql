-- ========================================
-- MIGRATION 000133: FIX SUMMARY RESULTS NET INCOME CALCULATION (DOWN)
-- ========================================
-- 
-- Purpose: Revertir corrección de ingreso neto (volver a versión anterior)
-- Date: 2025-10-11
-- Author: System

-- Revertir a la versión anterior (con el error)
...
by_crop AS (
  SELECT
    ...
    -- Versión anterior (con error): superficie × precio
    ...
    -- Versión anterior (con error): usa ingreso incorrecto
    (
      COALESCE(SUM(lb.seeded_area_ha * (
        SELECT COALESCE(cc.net_price, 0)
        FROM public.crop_commercializations cc
