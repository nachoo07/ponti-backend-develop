-- ========================================
-- MIGRACIÓN 000127: FIX v3_workorder_list UNION ALL (UP)
-- ========================================
-- 
-- Propósito: Restaurar UNION ALL de filas de labores + insumos (fix perdido en reorganización)
-- Referencia: Migración 000097 tenía este fix que se perdió en 000118
-- Fecha: 2025-10-08
-- Autor: Sistema
-- 
-- Problema: Vista solo muestra insumos, no muestra fila de labor
-- Solución: UNION ALL de:
--   1. Fila de LABOR (type_name='Labor', supply_name='-')
--   2. Filas de INSUMOS (una por cada supply)
-- 
...
-- ========================================
-- RECREAR VISTA v3_workorder_list
-- ========================================
-- FIX: UNION ALL para mostrar fila de labor + filas de insumos
...
labor_and_supplies AS (
  ...
  -- FIX: Solo traer filas donde existe un workorder_item
  WHERE w.deleted_at IS NULL AND wi.id IS NOT NULL

  UNION ALL

  -- Filas de LABORES SIN INSUMOS (workorders que NO tienen items)
  SELECT
    w.id,
    w.number,
    w.project_id,
  ...
  JOIN public.labors     lb ON lb.id = w.labor_id   AND lb.deleted_at IS NULL
  JOIN public.categories cat_lb ON cat_lb.id = lb.category_id AND cat_lb.deleted_at IS NULL
  -- Solo workorders que NO tienen insumos asociados
  WHERE w.deleted_at IS NULL
    AND NOT EXISTS (
      SELECT 1 FROM public.workorder_items wi 
      WHERE wi.workorder_id = w.id 
...
COMMENT ON VIEW public.v3_workorder_list IS 'Listado de workorders con UNION ALL: labor+insumos (si tiene) o solo labor (si no tiene)';
