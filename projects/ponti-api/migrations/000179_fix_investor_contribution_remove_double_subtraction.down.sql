-- ========================================
-- MIGRACIÓN 000179: FIX Investor Contribution Remove Double Subtraction (DOWN)
-- ========================================
--
...
-- Restaurar versión 000176 (con resta de movimientos internos)
...
SELECT
  ...
  -- AGROQUÍMICOS (versión 000176: con resta de movimientos internos)
  (COALESCE((
  ...
  -- FERTILIZANTES (versión 000176: con resta de movimientos internos)
  (COALESCE((
    SELECT SUM(sm.quantity * s.price)
    FROM public.supply_movements sm
    JOIN public.supplies s ON s.id = sm.supply_id AND s.deleted_at IS NULL
  ...
  -- SEMILLA (versión 000176: con resta de movimientos internos)
  (COALESCE((
    SELECT SUM(sm.quantity * s.price)
    FROM public.supply_movements sm
    JOIN public.supplies s ON s.id = sm.supply_id AND s.deleted_at IS NULL
...
'Categorías de aporte por proyecto para informe de inversores.';
