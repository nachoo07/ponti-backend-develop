-- ========================================
-- MIGRATION 000192: Align investor project base rent with rent_fixed_only (DOWN)
-- ========================================
--
-- Objetivo: Revertir la vista base del informe de aportes al cálculo anterior
--           usando v3_calc.rent_per_ha_for_lot.
--
...
SELECT
  ...
  COALESCE(SUM(v3_calc.rent_per_ha_for_lot(l.id) * l.hectares), 0::double precision)::numeric AS lease_fixed_total_usd,
  CASE
  ...
  CASE
    WHEN COALESCE(SUM(l.hectares), 0::double precision) > 0::double precision THEN
      COALESCE(SUM(v3_calc.rent_per_ha_for_lot(l.id) * l.hectares), 0::double precision) / SUM(l.hectares)
