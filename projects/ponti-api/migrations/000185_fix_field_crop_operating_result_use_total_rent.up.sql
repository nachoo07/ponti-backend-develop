-- ========================================
-- MIGRACIÓN 000185: FIX Field Crop Operating Result - Use Total Rent (UP)
-- ========================================
--
-- Propósito: Corregir cálculo de Resultado Operativo para usar arriendo TOTAL
-- Problema: La migración 000184 cambió arriendo a fixed_only en todos los cálculos,
--           pero el Resultado Operativo debe usar arriendo TOTAL (fijo + variable)
--           porque es el resultado final después de TODOS los costos.
-- Solución: En v3_report_field_crop_economicos:
--           - arriendo_usd: mantener fixed_only (para Total Invertido)
--           - resultado_operativo_usd: usar rent_per_ha_for_lot (fijo + variable)
-- Fecha: 2025-11-08
-- Autor: Sistema
--
-- Impacto: Control 11 pasará correctamente
--          Resultado Operativo reflejará costos reales finales
--
...
aggregated AS (
  SELECT
    ...
    -- Arriendo FIJO (para mostrar y para Total Invertido)
    SUM(v3_lot_ssot.rent_fixed_only_for_lot(lot_id) * hectares)::numeric AS rent_fixed_usd,
    -- ========================================
    -- FIX 000185: Arriendo TOTAL (para Resultado Operativo)
    -- ========================================
...
SELECT
  ...
  -- ========================================
  -- FIX 000185: Resultado Operativo usa Arriendo TOTAL
  -- ========================================
  -- Ingreso Neto - Costos Directos - Arriendo TOTAL - Administración
  -- Usa rent_total_usd (fijo + variable) no rent_fixed_usd
  ((production_tn * v3_lot_ssot.net_price_usd_for_lot(sample_lot_id)) - 
   (labor_costs_usd + supply_costs_usd) - rent_total_usd - administration_usd) AS resultado_operativo_usd,
  ((v3_core_ssot.safe_div(production_tn, sown_area_ha) * 
    v3_lot_ssot.net_price_usd_for_lot(sample_lot_id)) - 
...
'Vista 4/5: ECONÓMICOS - FIX 000185: arriendo_usd usa fixed, resultado_operativo_usd usa total.';
...
'Vista principal consolidada para reporte field-crop. FIX 000185: Resultado operativo usa arriendo total.';
