...
CREATE INDEX IF NOT EXISTS idx_labor_workorders_metrics_notdel 
  ON workorders(project_id, field_id, labor_id, effective_area) 
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_labor_workorder_items_notdel 
  ON workorder_items(workorder_id, supply_id) 
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_labor_supplies_notdel 
  ON supplies(id, price) 
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_labor_labors_notdel 
  ON labors(id, price) 
  WHERE deleted_at IS NULL;

-- Índice compuesto para JOINs frecuentes
CREATE INDEX IF NOT EXISTS idx_labor_workorders_composite 
  ON workorders(project_id, field_id, labor_id, effective_area) 
  WHERE deleted_at IS NULL AND effective_area > 0;

-- Índice para agregaciones por proyecto/campo
CREATE INDEX IF NOT EXISTS idx_labor_grouping 
  ON workorders(project_id, field_id, effective_area) 
  WHERE deleted_at IS NULL AND effective_area > 0;

-- Índice para optimizar agregaciones de supplies
CREATE INDEX IF NOT EXISTS idx_labor_workorder_items_supply 
