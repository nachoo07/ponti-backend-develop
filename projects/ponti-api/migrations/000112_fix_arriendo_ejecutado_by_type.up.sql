-- ========================================
-- MIGRACIÓN 000112: FIX ARRIENDO EJECUTADO BY TYPE (UP)
-- ========================================
-- 
-- Propósito: Corregir arriendo_ejecutados_usd según tipo de arriendo
-- Reglas:
--   - ARRIENDO FIJO (ID 3): ejecutado = lease_type_value * hectares
--   - ARRIENDO FIJO + % (ID 4): ejecutado = solo la parte fija (lease_type_value * hectares)
--   - % INGRESO NETO (ID 1): ejecutado = 0 (solo se ejecuta al final en cosecha)
--   - % UTILIDAD (ID 2): ejecutado = 0 (solo se ejecuta al final en cosecha)
-- Fecha: 2025-10-03
-- Autor: Sistema
...
SELECT
  ...
  -- ========================================
  -- ARRIENDO (EJECUTADOS + INVERTIDOS, STOCK = NULL) - CORREGIDO EN 000112
  -- ========================================
  -- Ejecutados: Depende del tipo de arriendo
  --   - FIJO (ID 3): lease_type_value * total_hectares
  --   - FIJO + % (ID 4): lease_type_value * total_hectares (solo parte fija)
  --   - % INGRESO NETO (ID 1): 0 (se ejecuta al final en cosecha)
  --   - % UTILIDAD (ID 2): 0 (se ejecuta al final en cosecha)
  COALESCE((
    SELECT CASE 
  ...
  -- Invertidos: siempre se calcula (representa el compromiso)
  COALESCE((SELECT f.lease_type_value * ph.total_hectares
            FROM public.fields f
            WHERE f.project_id = p.id AND f.deleted_at IS NULL
