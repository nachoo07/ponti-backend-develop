-- ========================================
-- MIGRATION 000087: FIX lot metrics calculation (UP)
-- ========================================
-- Purpose (EN): Fix lot metrics to use correct seeded/harvested areas
-- Fecha (ES): 2025-01-27
-- Autor (ES): System
-- Nota (ES): Nombres de columnas tal cual están hoy (sin renombres),
--            comentarios en español explicando cada cálculo.
...
SELECT
  ...
  COALESCE(v3_calc.direct_cost_for_lot(b.lot_id), 0)::numeric      AS direct_cost_usd,         -- costo directo total (USD)
  ...
  -- ############# 4) EN/ES: direct_cost_per_ha_usd (cost_per_hectare) / costo directo por hectárea ##############
  -- Fórmula (ES): direct_cost_total / worked_hectares
  -- Numerador: v3_calc.direct_cost_for_lot(b.lot_id) (USD)
  -- Denominador: superficie_total
  v3_calc.cost_per_ha(
    COALESCE(v3_calc.direct_cost_for_lot(b.lot_id), 0)::numeric,
    COALESCE(fta.total_hectares, 0)::numeric
  ) AS direct_cost_per_ha_usd,

  -- ############# 5) EN/ES: superficie_total / área total del campo (ha) ##############
  -- Fórmula (ES): suma de hectáreas (l.hectares) de todos los lotes activos del mismo campo
