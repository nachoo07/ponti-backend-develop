-- Migración 000054: Incluir costo administrativo en resultado operativo
-- Enfoque: Fórmula correcta: ((Ingreso Neto - Costos Directos - Costo Administrativo) / (Costos Directos + Costo Administrativo)) × 100
-- Comentarios en español según reglas del proyecto
-- NOTA: Los valores se guardan como positivos, el signo se interpreta en la aplicación
...
SELECT
  ...
  -- RESULTADO OPERATIVO: Guardar como valor absoluto (positivo)
  ABS((COALESCE(pi.total_income_usd, 0)) - (COALESCE(lc.executed_labors_usd,0) + COALESCE(sc.executed_supplies_usd,0)) - COALESCE(p.admin_cost, 0)) AS operating_result_usd,
  -- PORCENTAJE: Guardar como valor absoluto (positivo)
  ABS(CASE WHEN (COALESCE(lc.executed_labors_usd,0) + COALESCE(sc.executed_supplies_usd,0) + COALESCE(p.admin_cost, 0)) > 0
       THEN ((COALESCE(pi.total_income_usd, 0)) - (COALESCE(lc.executed_labors_usd,0) + COALESCE(sc.executed_supplies_usd,0)) - COALESCE(p.admin_cost, 0)) / (COALESCE(lc.executed_labors_usd,0) + COALESCE(sc.executed_supplies_usd,0) + COALESCE(p.admin_cost, 0)) * 100
  ...
  COALESCE(p.admin_cost, 0) AS admin_cost_usd,  -- Agregar costo administrativo para referencia
  -- FLAG para indicar si es pérdida (0) o ganancia (1)
  CASE WHEN (COALESCE(pi.total_income_usd, 0)) - (COALESCE(lc.executed_labors_usd,0) + COALESCE(sc.executed_supplies_usd,0)) - COALESCE(p.admin_cost, 0) < 0 THEN 0 ELSE 1 END AS is_profit
FROM projects p
