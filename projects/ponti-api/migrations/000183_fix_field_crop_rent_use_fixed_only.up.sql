-- ========================================
-- MIGRACIÓN 000183: FIX Field Crop Report - Use Fixed Rent Only (UP)
-- ========================================
--
-- Propósito: Corregir cálculo de Total Invertido en informe de campo y cultivo
-- Problema: v3_report_field_crop_rentabilidad usa rent_per_ha_for_lot() que incluye:
--           - Arriendo fijo + Arriendo variable (% sobre ingresos)
--           Esto infla incorrectamente el "Total Invertido" porque el arriendo
--           variable NO es inversión inicial (se paga según resultados)
-- Solución: Usar rent_fixed_only_for_lot() que solo considera arriendo capitalizable:
--           - Tipo 1 (% ingreso): 0
--           - Tipo 2 (% resultado): 0
--           - Tipo 3 (Fijo): lease_type_value
--           - Tipo 4 (Fijo + %): solo lease_type_value (sin %)
-- Fecha: 2025-11-08
-- Autor: Sistema
--
-- Impacto: Total Invertido mostrará solo inversión inicial real
--          Ejemplo SJDD: de 132,821.60 USD a 89,900.00 USD (-42,921.60 USD)
--          Control 3 de integridad pasará correctamente
--
...
aggregated AS (
  SELECT
    ...
    -- ========================================
    -- FIX 000183: ARRIENDO SOLO FIJO
    -- ========================================
    -- ANTES (INCORRECTO): rent_per_ha_for_lot() incluía % variable
    -- AHORA (CORRECTO): rent_fixed_only_for_lot() solo inversión capitalizable
...
SELECT
  ...
  -- Total Invertido = Gastos Directos + Arriendo FIJO + Administración
  ...
  -- Rinde de Indiferencia = Total Invertido/Ha / Precio Neto
  v3_core_ssot.safe_div(
...
'Vista 5/5: RENTABILIDAD - FIX 000183: Arriendo usa rent_fixed_only_for_lot() para Total Invertido correcto.';
...
'Vista principal consolidada para reporte field-crop. FIX 000183: Total Invertido usa arriendo fijo solamente.';
