-- ========================================
-- MIGRATION 000137: FIX SUMMARY RESULTS DIRECT COSTS CALCULATION (UP)
-- ========================================
-- 
-- Purpose: Corregir cálculo de costos directos en v3_report_summary_results_view
-- Date: 2025-10-12
-- Author: System
-- 
-- Problema identificado:
--   La vista usa v3_calc.direct_cost_for_lot que incluye movimientos internos
--   Esto infla artificialmente los costos directos ejecutados
--
-- Corrección:
--   Usar las mismas funciones que v3_workorder_metrics (card de workorders):
--   - v3_lot_ssot.labor_cost_for_lot (labores ejecutadas)
--   - v3_lot_ssot.supply_cost_for_lot_base (insumos ejecutados SIN movimientos internos)
--
-- Impacto:
--   - Costos directos mostrarán solo valores ejecutados en workorders
--   - Se eliminan movimientos internos que inflaban los costos
--   - Consistencia con la card de "Costos directos" en órdenes de trabajo
--
-- Note: Code in English, comments in Spanish.

-- -------------------------------------------------------------------
-- v3_report_summary_results_view: resumen de resultados generales por cultivo (CORREGIDO)
-- -------------------------------------------------------------------
...
by_crop AS (
  SELECT
    ...
    -- Ingreso neto total (tons × net_price)
    COALESCE(SUM(v3_calc.income_net_total_for_lot(lb.lot_id)), 0)::numeric AS net_income_usd,
    
    -- CORREGIDO: Costos directos ejecutados (mismo cálculo que v3_workorder_metrics)
    -- Solo workorders, SIN movimientos internos
    COALESCE(SUM(
